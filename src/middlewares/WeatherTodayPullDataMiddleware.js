...
const WeatherTodayPullDataMiddleware = () => (next) => async (action) => {
    ...
    case SEARCH_CURRENT_WEATHER_INFO: {
        ...
        await axios
            .post(`https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&units=metric&appid=${env.API_KEY}`)
            .then((response) => configurePayload(response));
    } break;
    case SEARCH_POLLUTION_INFO: {
        const { latitude, longitude } = action.payload;
        await axios
            .get(`http://api.openweathermap.org/data/2.5/air_pollution?lat=${latitude}&lon=${longitude}&appid=${env.API_KEY}`)
            .then((response) => configurePayload(response));
    } break;
    default:
    }
